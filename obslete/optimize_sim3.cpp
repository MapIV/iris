...
int main()
{
  ...
  for (size_t i = 0; i < true_points.size(); ++i) {
    ...
    pt0 = true_points[i];
    pt1 = vp0->estimate().inverse().map(true_points[i]);
  ...
  g2o::VertexSim3Expmap* vc = dynamic_cast<g2o::VertexSim3Expmap*>(optimizer.vertices().find(0)->second);
  ...
  // clang-format off
  std::cout << std::endl
            << "vertex should be near 0,0,1, and scale should be near 1" 
            << std::endl;
  std::cout <<"translation: "
            << dynamic_cast<g2o::VertexSim3Expmap*>(optimizer.vertices().find(0)->second)->estimate().translation().transpose()
            << std::endl;
  std::cout <<"scale: "
            << dynamic_cast<g2o::VertexSim3Expmap*>(optimizer.vertices().find(0)->second)->estimate().scale()
            << std::endl;
  // clang-format on
}