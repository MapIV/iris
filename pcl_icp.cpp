...
int main(int, char**)
{
  ...
  for (std::size_t i = 0; i < cloud_in->points.size(); ++i) {
    cloud_in->points[i].x = static_cast<float>(1024 * rand() / (RAND_MAX + 1.0));
    cloud_in->points[i].y = static_cast<float>(1024 * rand() / (RAND_MAX + 1.0));
    cloud_in->points[i].z = static_cast<float>(1024 * rand() / (RAND_MAX + 1.0));
  }
